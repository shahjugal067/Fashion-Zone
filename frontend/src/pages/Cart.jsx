             ...
             sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
                <div className='flex items-start gap-6'>
                    <img src={productData.image[0]} alt=""
                     className='w-20 h-30 sm:w-24'/>
                     <div>
                      <p className='text-sm sm:text-lg'>{productData.name}</p>
                      <div className='flex items-center gap-5 mt-2'>
                          <p>{currency}{productData.price}</p>
                          <p className='px-2 py-1 bg-gray-200'>{item.size}</p>
                      </div>
                     </div>
                </div>
                ...
                :  updateQuantity(item._id, item.size, Number(e.target.value))}
                 ...
                 <Trash onClick={()=> updateQuantity(item._id, item.size, 0)}
      ...
      <div className='flex justify-end my-20'>
        <div className='w-full sm:w-[450px]'>
          ...
          <div className='w-full text-end'>
            ...
            className='bg-yellow-500 px-10 my-8 py-3 text-white'>
              Please Checkout</button>
          </div>
        </div>
